...
-- content
local function source_include(filepath, startLine, endLine, dedentLine, lang, filename, line_number, code_filename)
	local add_source = {
...
function Div(el)
	if el.attributes["add-from"] then
		...
		local div = el:walk(
			source_include(filepath, startLine, endLine, dedent_line, lang, filename, line_number, code_filename)
		)
		return div
	end
end
