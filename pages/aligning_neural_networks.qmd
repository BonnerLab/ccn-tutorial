...
- Compute activation and weight covariance spectra
...
- Generate a new network from the ensemble of networks with similar performance
...
![samples from CIFAR-10](https://production-media.paperswithcode.com/datasets/4fdf2b82-2bc3-4f97-ba51-400322b228b1.png)
...
We begin with a barebones feedforward network architecture with two hidden layers. It computes the function
$$ f(x) = \theta \sigma(W_2 \sigma(W_1 x))$$

with:

- $x \in \mathbb R^d$ is an input image reshaped as a vector ($d = 32 \times 32$),
- $W_1 \in \mathbb R^{n_1 \times d}$ the weights of the $n_1$ neurons in the first layer,
- $W_2 \in \mathbb R^{n_2 \times n_1}$ the weights of the $n_2$ neurons in the second layer,
- $\theta \in \mathbb R^{c \times n_2}$ the weights of the classifier, which predicts log-likelihoods for the $c = 10$ classes,
...
At each layer, we can measure the dimensionality of the corresponding hidden representation with a PCA. We thus compute the covariance of the representation. Let $\varphi(x)$ denote the representation of the input $x$ at the given layer, we compute:
$$ \mathbb E_x\left[\varphi(x) \, \varphi(x)^\top\right]. $$

*(We shall act as if the mean $\mathbb E_x \left[ \varphi(x) \right]$ was zero for simplicity. Typically, this is enforced by a normalization layer such as [batch normalization](https://pytorch.org/docs/stable/generated/torch.nn.BatchNorm2d.html) or [layer normalization](https://pytorch.org/docs/stable/generated/torch.nn.LayerNorm.html).)*
...
As another way of evaluating the important of each principal component, we can perform *clipping*. This consists in inserting an operation in the middle of the network, which projects the hidden representation on its first $k$ principal components. We can then evaluate the accuracy of the network as a function of $k$, which allows measuring the cumulative "information" stored in the first $k$ PCs. 
...
for layer_idx in range(2):
    ...
    activation_eigenvalues = activation_eigenvalues.flip(
        -1
    )  # (num_eig,), descending order
    activation_eigenvectors = activation_eigenvectors.flip(
        -1
    ).T  # (num_eig, dim), descending order
...
Further, they can be somewhat reduced in dimension at little cost in accuracy (note that rank is in log scale).
...
$$ \min_A \mathbb E_x[\|A\varphi(x) - \psi(x)\|^2] \quad\text{such that}\quad A^\top A = {\rm Id} $$
...
print(f"Generating {state_dict_keys}")
reference_accuracy, generated_accuracy = generate_network(cnn_model, state_dict_keys, train_loader, val_loader)
print(f"Accuracy after sampling: {generated_accuracy:.1f}% (accuracy after training: {reference_accuracy:.1f}%)")
```
