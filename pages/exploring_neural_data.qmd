...
::: {.content-visible when-format="html" .callout-tip title="Run this notebook interactively!"}
...
This data contains neural responses to 872 images from ~15,000 voxels reliably modulated by the visual stimuli during the NSD experiment.

::: {.callout-note}
We utilized the NSD single-trial betas, preprocessed in 1.8-mm volume space and denoised using the GLMdenoise technique (version 3; “betas_fithrf_GLMdenoise_RR”). The betas were converted to z-scores within each scanning session, and we computed the average betas for each NSD stimulus across repetitions.
:::
...
### Visualizing PCs by finding extreme images
...
```{python}
# | code-summary: View eigenvectors on brain surface
# | fig-column: page
# | fig-align: center
...
On this plot, there appears to be negligible variance after about rank 20. It is a common practice to drop the rest of higher-rank PCs. However, before doing so, let's try visualizing the spectrum on a logarithmic scale for both axes:
...
umap = UMAP(n_components=2, metric="euclidean", random_state=0)
