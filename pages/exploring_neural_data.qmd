...
Interpreting PCs can be challenging especially when we rely solely on visual inspection. One of the reasons is that many features in nature are non-negative. This difficulty arises, in part, because many natural features are non-negative. As a result, methods like nonnegative matrix factorization (NMF) often offer more interpretable dimensions than PCA.
...
for component in range(2):
    view_images_at_poles(data_pca[:, component], stimuli=stimuli, label=f"component {rank}")
```
