...
There are some simple ways to visualize and interpret the principle components.
...
On this plot, it looks like that the first few PCs have substantial variance while the rest are negligable, which suggests a low-dimensional structure. However, when dealing with high-dimensional data that span several orders of magnitude, it's more insightful to use a logarithmic scale. We can examine many key statistical trends more clearly across the orders. So let's try visualizing the spectrum on a logarithmic scale for both axes:
...
On a log-log scale, the spectrum shows a trend similar to a linear relationship, suggesting that the eignenspectrum might be a power law distribution:

$$\log{\lambda_\text{rank}} \approx \alpha \log{\left( \text{rank} \right)} + c \implies \lambda_\text{rank} \propto \left( \text{rank} \right)^\alpha$$

There is information spanning across all ranks as we find no obvious cut-off point, this distributions. So the number of effect dimensions here is likely much higher than what we would speculate from the linear scale.
...
This demonstrates that the high-rank, low-frequency words also carries meaningful information. Similarly, we will likely also lose information if we try to reconstruct neural data wiht just the high-variance PCs.
